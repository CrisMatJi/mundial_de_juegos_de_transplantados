...
const fallbackSemiFinals = [
  { label: 'Semifinal 1', top: 'Inglaterra', bottom: 'Gales' },
  { label: 'Semifinal 2', top: 'México', bottom: 'España' },
]
...
function App() {
  ...
  useEffect(() => {
    fetchOfficialData().then(({ matches: officialMatches, teams, groups: officialGroups }) => {
      ...
      const knockout = officialMatches.filter((match: { stage: string }) => match.stage === 'quarterfinal' || match.stage === 'semifinal')
      ...
      setSemiFinals(knockout.filter((match: { stage: string }) => match.stage === 'semifinal').map((match: { bracket_position: number; home_team_id: string | null; away_team_id: string | null }) => ({ label: `Semifinal ${match.bracket_position}`, top: matchName(match.home_team_id), bottom: matchName(match.away_team_id) })))
    ...
    <main className="app-shell">
      ...
      <section className="content-wrap">
        ...
        <div className="dashboard-grid">
          ...
          <aside className="panel live-panel"><div className="panel-heading"><div><p className="section-kicker">SEÑAL OFICIAL</p><h2>Ahora en directo</h2></div><span className="on-air"><Radio size={13} /> ON AIR</span></div><div className="video-frame"><iframe title="España contra Chile en directo" src="https://www.youtube.com/embed/5OQ4DrKJy_s?rel=0" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /><div className="video-unavailable"><CirclePlay size={38} /><strong>España · Chile</strong><span>Si YouTube bloquea la inserción, abre la emisión directamente.</span><a href="https://www.youtube.com/watch?v=5OQ4DrKJy_s" target="_blank" rel="noreferrer">Ver emisión en YouTube <ExternalLink size={14} /></a></div></div><div className="live-footer"><Tv size={17} /><span><strong>Emisión oficial</strong><small>Si el reproductor falla, usa el enlace directo</small></span><a href="https://www.youtube.com/watch?v=5OQ4DrKJy_s" target="_blank" rel="noreferrer" aria-label="Abrir emisión de España contra Chile en YouTube"><ArrowUpRight size={17} /></a></div></aside>
        </div>

        <section className="panel bracket-panel" id="bracket"><div className="panel-heading"><div><p className="section-kicker">FASE FINAL</p><h2>Camino al título</h2></div><span className="bracket-note">8 equipos · 4 cruces</span></div><div className="bracket-layout"><div className="bracket-rounds"><div className="round-label">CUARTOS DE FINAL <span>JUE 17 SEP · ACTUALIZADO</span></div>{bracket.map((game) => <div className="bracket-match" key={game.label}><span className="bracket-label">{game.label}</span><div className={game.top.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span>{game.top}</span><b>{game.topScore}</b></div><div className={game.bottom.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span>{game.bottom}</span><b>{game.bottomScore}</b></div><i /></div>)}</div><div className="semi-round"><div className="round-label">SEMIFINALES <span>VIE 18 SEP · 08:00</span></div>{semiFinals.map((game) => <div className="semi-match" key={game.label}><span className="bracket-label">{game.label}</span><div className={game.top.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span>{game.top}</span><b>—</b></div><div className={game.bottom.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span>{game.bottom}</span><b>—</b></div></div>)}</div><div className="bracket-final"><span className="round-label">FINAL <span>VIE 18 SEP · 13:45</span></span><div className="final-card"><Trophy size={25} /><strong>Campeón del<br />mundo 2026</strong><span>El partido decisivo</span></div></div></div><p className="bracket-footnote"><Flag size={14} /> Datos oficiales de Gateway. España se enfrentará a México en semifinales.</p></section>
      </section>
