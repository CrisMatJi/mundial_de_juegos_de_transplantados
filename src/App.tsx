...
const semiFinals = [
  { label: 'Semifinal 1', top: 'Ganador Cuartos 1', bottom: 'Ganador Cuartos 2' },
  { label: 'Semifinal 2', top: 'Ganador Cuartos 3', bottom: 'Ganador Cuartos 4' },
]
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content-wrap">
        ...
        <section className="panel bracket-panel" id="bracket"><div className="panel-heading"><div><p className="section-kicker">FASE FINAL</p><h2>Camino al título</h2></div><span className="bracket-note">8 equipos · 4 cruces</span></div><div className="bracket-layout"><div className="bracket-rounds"><div className="round-label">CUARTOS DE FINAL <span>JUE 17 SEP · EN JUEGO</span></div>{bracket.map((game) => <div className="bracket-match" key={game.label}><span className="bracket-label">{game.label}</span><div className={game.top.includes('ESP') ? 'bracket-team spain' : 'bracket-team'}><span>{game.top}</span><b>—</b></div><div className="bracket-team"><span>{game.bottom}</span><b>—</b></div><i /></div>)}</div><div className="semi-round"><div className="round-label">SEMIFINALES <span>VIE 18 SEP · 08:00</span></div>{semiFinals.map((game) => <div className="semi-match" key={game.label}><span className="bracket-label">{game.label}</span><div className="bracket-team"><span>{game.top}</span><b>—</b></div><div className="bracket-team"><span>{game.bottom}</span><b>—</b></div></div>)}</div><div className="bracket-final"><span className="round-label">FINAL <span>VIE 18 SEP · 13:45</span></span><div className="final-card"><Trophy size={25} /><strong>Campeón del<br />mundo 2026</strong><span>El partido decisivo</span></div></div></div><p className="bracket-footnote"><Flag size={14} /> España disputa ahora los cuartos contra Chile. Los ganadores avanzan a las semifinales.</p></section>
      </section>
