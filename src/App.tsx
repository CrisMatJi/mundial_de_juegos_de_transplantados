...
const officialName = (name: string) => ({ Spain: 'España', Wales: 'Gales', England: 'Inglaterra', Mexico: 'México', Germany: 'Alemania', Italy: 'Italia', France: 'Francia', Ireland: 'Irlanda', 'United States': 'Estados Unidos' }[name] ?? name)
const countryCode = (name: string) => ({ España: 'es', 'Estados Unidos': 'us', Irlanda: 'ie', México: 'mx', Francia: 'fr', Chile: 'cl', Inglaterra: 'gb-eng', Gales: 'gb-wls', Alemania: 'de', Italia: 'it', Colombia: 'co' }[name] ?? 'un')
const teamFlag = (name: string) => countryCode(name)
const stageLabel = (stage: string) => ({ group: 'Grupo C', quarterfinal: 'Cuartos de final', semifinal: 'Semifinal' }[stage] ?? stage)
const matchStatus = (status: string) => status === 'finished' ? 'Finalizado' : status === 'live' ? 'En directo' : 'Próximo'

function CountryFlag({ name, className = '' }: { name: string; className?: string }) {
  return <img className={`country-flag ${className}`} src={`https://flagcdn.com/w40/${countryCode(name)}.png`} alt={`Bandera de ${name}`} loading="lazy" />
}
...
function App() {
  ...
  useEffect(() => {
    fetchOfficialData().then(({ matches: officialMatches, teams, groups: officialGroups }) => {
      ...
      const officialSpainMatches = officialMatches.filter((match: { stage: string; home_team_id: string | null; away_team_id: string | null }) => ['group', 'quarterfinal', 'semifinal'].includes(match.stage) && (matchName(match.home_team_id) === 'España' || matchName(match.away_team_id) === 'España'))
      ...
      if (officialSpainMatches.length > 0) {
        setMatches(officialSpainMatches.map((match: { stage: string; home_team_id: string | null; away_team_id: string | null; home_score: number | null; away_score: number | null; kickoff_at: string; status: string }) => {
          const isHome = matchName(match.home_team_id) === 'España'
          const opponent = isHome ? matchName(match.away_team_id) : matchName(match.home_team_id)
          const ownScore = isHome ? match.home_score : match.away_score
          const opponentScore = isHome ? match.away_score : match.home_score
          const kickoff = new Date(match.kickoff_at)
          return { day: new Intl.DateTimeFormat('es-ES', { weekday: 'short', day: '2-digit', month: 'short' }).format(kickoff).toUpperCase(), date: new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'long' }).format(kickoff), time: match.status === 'live' ? 'Ahora' : new Intl.DateTimeFormat('es-ES', { hour: '2-digit', minute: '2-digit' }).format(kickoff), opponent, opponentFlag: teamFlag(opponent), score: ownScore === null || opponentScore === null ? '— —' : `${ownScore} — ${opponentScore}`, status: matchStatus(match.status), stage: stageLabel(match.stage) } as Match
        }))
      }
      ...
      <section className="content-wrap">
        ...
        <div className="match-strip" id="partidos">
          {filteredMatches.map((match, index) => <article className={match.status === 'Fase final' || match.status === 'En directo' ? 'match-card next-match' : 'match-card'} key={`${match.day}-${match.opponent}`}>
            ...
            <div className="match-body"><div className="country"><CountryFlag name="España" /><strong>España</strong></div><div className="match-result">{match.score ? <strong>{match.score}</strong> : <span className="tbd">POR CONFIRMAR</span>}<small>{match.time} · {index === matches.length - 1 ? 'Campo 1' : 'Campo 2'}</small></div><div className="country opponent"><CountryFlag name={match.opponent} /><strong>{match.opponent}</strong></div></div>
        ...
        <div className="dashboard-grid">
          ...
          <aside className="panel live-panel"><div className="panel-heading"><div><p className="section-kicker">SEÑAL OFICIAL</p><h2>Ahora en directo</h2></div><span className="on-air"><Radio size={13} /> ON AIR</span></div><div className="video-frame"><iframe title="Directo del canal WTGF" src={`https://www.youtube-nocookie.com/embed/live_stream?channel=${YOUTUBE_CHANNEL_ID}&rel=0`} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /><div className="video-unavailable"><CirclePlay size={38} /><strong>Canal oficial WTGF</strong><span>Si el directo no se puede insertar, ábrelo directamente en YouTube.</span><a href={YOUTUBE_CHANNEL_URL} target="_blank" rel="noreferrer">Abrir directo del canal <ExternalLink size={14} /></a></div></div><div className="live-footer"><Tv size={17} /><span><strong>World Transplant Games Federation</strong><small>Directo del canal oficial</small></span><a href={YOUTUBE_CHANNEL_URL} target="_blank" rel="noreferrer" aria-label="Abrir el canal oficial de WTGF en YouTube"><ArrowUpRight size={17} /></a></div></aside>
        </div>

        <section className="panel bracket-panel" id="bracket"><div className="panel-heading"><div><p className="section-kicker">FASE FINAL</p><h2>Camino al título</h2></div><span className="bracket-note">8 equipos · 4 cruces</span></div><div className="bracket-layout"><div className="bracket-rounds"><div className="round-label">CUARTOS DE FINAL <span>JUE 17 SEP · ACTUALIZADO</span></div>{bracket.map((game) => <div className="bracket-match" key={game.label}><span className="bracket-label">{game.label}</span><div className={game.top.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span><CountryFlag name={game.top} />{game.top}</span><b>{game.topScore}</b></div><div className={game.bottom.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span><CountryFlag name={game.bottom} />{game.bottom}</span><b>{game.bottomScore}</b></div><i /></div>)}</div><div className="semi-round"><div className="round-label">SEMIFINALES <span>VIE 18 SEP · FINALIZADAS</span></div>{semiFinals.map((game) => <div className="semi-match" key={game.label}><span className="bracket-label">{game.label}</span><div className={game.top.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span><CountryFlag name={game.top} />{game.top}</span><b>{game.topScore}</b></div><div className={game.bottom.includes('España') ? 'bracket-team spain' : 'bracket-team'}><span><CountryFlag name={game.bottom} />{game.bottom}</span><b>{game.bottomScore}</b></div></div>)}</div><div className="bracket-final"><span className="round-label">FINAL <span>VIE 18 SEP · 13:45</span></span><div className="final-card"><Trophy size={25} /><div className="final-teams"><CountryFlag name={finalMatch.top} /><strong>{finalMatch.top}</strong><span>{finalMatch.topScore || 'vs'}</span><strong>{finalMatch.bottom}</strong><CountryFlag name={finalMatch.bottom} /></div><span>{finalMatch.status} · partido decisivo</span></div></div></div><p className="bracket-footnote"><Flag size={14} /> Datos oficiales de Gateway. España jugará la final contra Inglaterra.</p></section>
      </section>
